...
int MaxCount(int* arr, int n, int w1_r, int w2_r, int w3_r ,int i)
{
    ...
    if (w1_r >= arr[i])
        fill_w1 = 1 +MaxCount(arr, n, w1_r - arr[i], w2_r, w3_r , i + 1);
 
    if (w2_r >= arr[i])
        fill_w2 = 1 +MaxCount(arr, n, w1_r, w2_r - arr[i], w3_r, i + 1);
    
    if (w3_r >= arr[i])
        fill_w3 = 1 +MaxCount(arr, n, w1_r, w2_r , w3_r- arr[i], i + 1);
...
int main()
{
    ...
    cin>>w3;
    // int w1 = 4, w2 = 2, w3= 1;
    cout <<"The max no of weights trucks can load is:"<<MaxCount(arr, n, w1, w2, w3, 0);
    return 0;
}